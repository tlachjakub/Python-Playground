...
for i in glob.glob('/Work/MyProjects/Python/filter-yellow/input-images/*.png'):
	#temp = cv2.imread(img)
	# TODO: Compile the image into the binary


	input_images.append(np.fromfile(i, dtype='uint8'))
	imgCount += 1
...
filterCount = 0
for i in glob.glob('/Work/MyProjects/Python/filter-yellow/output-images/*.png'):
	...
	output_images.append(np.fromfile(i, dtype='uint8'))
	filterCount += 1
